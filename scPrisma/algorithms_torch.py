...
def sga_matrix_momentum_torch(A, E, V, iterNum=400, batch_size=20, lr=0.1, gama=0.9 , verbose=True):
    '''
    :param A: gene expression matrix
    :param E: permutation matrix initial value
    :param V: Eigenvectors matrix multiple by sqrt of diagonal eigenvalues matrix
...
def reconstruction_cyclic_torch(A, iterNum=300, batch_size=None, gama=0.5, lr=0.1 , verbose=True , final_loss=False):
    '''
    Cyclic reorder rows using stochastic gradient ascent
    :param A: gene expression matrix
    :param iterNum:  iteration number
    ...
    E = torch.ones((n,n))
    E = E.float()
    E = sga_matrix_momentum_torch(A, E=E / n, V=V, iterNum=iterNum, batch_size=batch_size, gama=gama, lr=lr , verbose=verbose)
    E_recon = reconstruct_e(E.numpy())
    if final_loss:
        value, grad = fAndG_matrix_torch(A=((1/A.shape[0])*A), E=E_recon, V=V.T)
        return E, E_recon , value
    return E.numpy(), E_recon
...
def filter_non_cyclic_genes_torch(A, regu=0.1, lr=0.1, iterNum=500):
    ...
    D = gradient_ascent_filter_matrix_torch(A, D=D, U=U, regu=regu, lr=lr, iterNum=iterNum)
    return D
...
def gradient_ascent_filter_matrix_torch(A, D, U, ascent=1, lr=0.1, regu=0.1, iterNum=400):
    '''
    :param A: gene expression matrix
...
def enhancement_cyclic_per_gene1(A, regu=0.1, iterNum=300):
    ''' Enhancement of cyclic signal
    :param A: Gene expression matrix (reordered according to cyclic ordering)
    :param regu: regularization coefficient
    :param iterNum: iteration number
...
def filtering_cyclic_torch(A, regu=0.1, iterNum=300, verbosity = 25 , error=10e-7, optimize_alpha=True, line_search=False):
    ...
    if line_search:
        ...
        F = gradient_descent_full_torch(A,F,V=V,regu=regu,epsilon=0.1,iterNum=iterNum)
    return F
...
def gradient_descent_full_torch(A, F, V, regu, epsilon=0.1, iterNum=400 , regu_norm ='L1'):
    j = 0
    epsilon_t = epsilon
    while j < iterNum:
        if j % 100 == 1:
            print("Iteration number: " + str(j))
        epsilon_t *= 0.995
        grad = G_full_torch(A=A, B=F, V=V, alpha=regu, regu_norm=regu_norm)
        F = F - epsilon_t * grad
        F = torch.clip(F,0,1)
        j += 1
    return F
...
def G_full_torch(A, B, V, alpha, regu_norm='L2'):
    '''
    :param A: Gene expression matrix
    :param B: filtering matrix
    :param V: spectral matrix
    ...
    if regu_norm=='L1':
        T_0 = (A * B)
        gradient = ((2 * (((V).mm(V.T)).mm(T_0) * A)) - ((alpha ) * torch.sign(B)))
    else:
        T_0 = (A * B)
        t_1 = torch.norm(A * B, 'fro')
        gradient = ((2 * (((V).mm(V.T)).mm(T_0) * A))- ((alpha / t_1) * B))
    return gradient
...
def BBS_torch(E, iterNum=1000):
    ...
    '''
    n = E.shape[0]
    prev_E = torch.empty(E.shape)
    I = torch.eye(n)
    ones_m = torch.ones((n, n))
    for i in range(iterNum):
        ...
        if i % 15 == 1:
            if torch.norm(E - prev_E) < ((10e-6) * n):
                break
    return E
